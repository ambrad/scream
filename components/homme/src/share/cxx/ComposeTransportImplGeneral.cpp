...
{
  fprintf(stderr,"amb> ComposeTransportImpl::ctor\n");
  setup();
}
...
ComposeTransportImpl::ComposeTransportImpl (const int num_elems)
  : m_tp_ne(1,1,1), m_tp_ne_qsize(1,1,1), m_tp_ne_hv_q(1,1,1), // throwaway settings
    m_tu_ne(m_tp_ne), m_tu_ne_qsize(m_tp_ne_qsize), m_tu_ne_hv_q(m_tp_ne_hv_q)
{
  fprintf(stderr,"amb> ComposeTransportImpl::ctor(num_elems)\n");
  nslot = calc_nslot(m_geometry.num_elems());
}

void ComposeTransportImpl::setup () {
  fprintf(stderr,"amb> ComposeTransportImpl::setup\n");
...
void ComposeTransportImpl::reset (const SimulationParams& params) {
  fprintf(stderr,"amb> ComposeTransportImpl::reset\n");
  ...
  if (m_data.nelemd == num_elems && m_data.qsize == params.qsize) {
    fprintf(stderr,"amb> ComposeTransportImpl::reset early return\n");
    return;
  }
  ...
  fprintf(stderr,"amb> ComposeTransportImpl::reset done\n");
}

  ...
  Kokkos::parallel_for(Kokkos::TeamThreadRange(team, nlev_pack), [&] (const Int& k) {
    ...
    Spack Skew_w(0), w1_1(w_first), w1_2(w_first), w2_1(0), w2_2(0), a(0.5);
    {
      ...
      if (condition.any()) {
        ...
        const Scalar sqrtw2t(std::sqrt(one_m_tmp_val));
    ...
    {
      const Smask condition = (thlsec > (thl_tol*thl_tol) &&
                               ekat::abs(w1_2 - w1_1) > w_thresh);
      if (condition.any()) {
    ...
    EKAT_KERNEL_REQUIRE(!is_neg_Tl1_1.any());
    EKAT_KERNEL_REQUIRE(!is_neg_Tl1_2.any());
          ...
          s2(0), std_s2(0), qn2(0), C2(0), ql2(0);
    {
      ...
      std_s1 = ekat::sqrt(ekat::max(0,
                                    ...
                                    + ekat::square(cqt1)*qw2_1
                                    - 2*cthl1*sqrtthl2_1*cqt1*sqrtqw2_1*r_qwthl_1));
      const auto std_s1_not_small = std_s1 > std::sqrt(2.23e-308) * 100;
      ...
      if (std_s1_C1_not_small.any()) {
        qn1.set(std_s1_C1_not_small,
                s1*C1+(std_s1/sqrt2pi)*ekat::exp(-sp(0.5)*ekat::square(s1/std_s1)));
      }
      ...
      if (nequal.any()) {
        ...
        if (nequal_std_s2_C2_not_small.any()) {
          qn2.set(nequal_std_s2_C2_not_small,
                  s2*C2+(std_s2/sqrt2pi)*ekat::exp(-sp(0.5)*ekat::square(s2/std_s2)));
        }

...
public:
  ...
  struct SHOCPreprocess {
    ...
    void operator()(const Kokkos::TeamPolicy<KT::ExeSpace>::member_type& team, shoc::Result& r) const {
      ...
      Kokkos::single(
        Kokkos::PerTeam(team),
        [&] () {
          shoc::combine(r.v[0], surf_sens_flux(i));
          shoc::combine(r.v[1], rrho_i(i,nlevi_v)[nlevi_p]);
          shoc::combine(r.v[2], inv_exner_int_surf);
          shoc::combine(r.v[3], wpthlp_sfc(i));
        });
